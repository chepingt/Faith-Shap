...
class CurveData():
    ...
    def get_average_stop_time(self, desired_l2, larger_than_1000 = False):
        ...
        l2s = []
        '''
        for value_traj, l2_traj in zip(self.data['values'], self.data['l2'] ):
            stop_evals.append(self.checkpoints[-1]) 
            l2s.append(l2_traj[-1]) 
            for i, x in enumerate(value_traj):
                if i == len(value_traj):
                    break
                y = value_traj[i+1]
                dis = np.linalg.norm(x-y) ** 2 / len(x)
                if dis < desired_l2:
    ...
    def get_mean_and_std_seqs(self, seqs):
        ...
        def get_mean_and_std(data):
            a = 1.0 * np.array(data)
            #m, percentile5, percentile95 = np.mean(a), np.percentile(a, 5), np.percentile(a, 95)
            #return m, percentile5, percentile95
            ...
            for inter in list(combinations(range(self.d), self.l)):
                ...
                if self.I.n_flag + eval_cnt > self.checkpoints[now_cpt]:
                    ...
                    if now_cpt >= len(self.checkpoints):
                        break
                '''
                n_evaluations = self.I.n_flag + eval_cnt 
                if desired_l2 > 0:
                    if n_evaluations > 100 * m :
                        m += 1
                '''
...
if __name__ == '__main__':
    ...
    fig_name = 'bank_top10_{}instance_d{}_{}_l{}_n_trials{}_lasso{}_ridge{}_seed{}_std3.png'.format(args.n_intn, args.min_d,\
    ...
    if os.path.exists(fig_path): 
        temp(fig_path)
        exit()
    #instances10_12 = load_perturbations(data_path_10_12, args.min_d, args.max_d)
    instances = load_perturbations(args.data_path, 12, 20)
